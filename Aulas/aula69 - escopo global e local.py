...
A palavra global faz uma variável do escopo externo ser a mesmo no escopo interno.
"""
#debug
x = 1 # 'x' do módulo
...
def escopo():# o que eu defino dentro da função não pode ser alterada fora da função
    ...
    def outra_funcao():
        x = 11# se eu não tivesse esse 'x' aqui, eu teria como valor o 'x' mais próximo dessa função, que seria o 'x' da função escopo().
